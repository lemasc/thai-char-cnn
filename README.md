...
trains only that row. `model` is `small_cnn` (32 px) or `resnet18` (64 px, one-channel stem;
`pretrained="IMAGENET1K_V1"` starts from ImageNet weights, `None` from random ones). `TRAIN_VERSION`
in `train.py` is bumped by hand when a change alters what an existing config produces; adding a model
or a config key with a neutral default (`IDENTITY_NEUTRAL`) keeps every earlier run. `RETRAIN=1`
